...
public class AvcDecoder extends Decoder {
    ...
    @SuppressLint("NewApi")
    public AvcDecoder(int width, int height, int frameRate, int bitRate, Surface surface) throws IOException {
        mFrameRate = frameRate;
        mSurface = surface;

        mMediaCodec = MediaCodec.createDecoderByType(Config.TYPE_H264);

        mMediaFormat = MediaFormat.createVideoFormat(Config.TYPE_H264, width, height);
